...
import { CheckCircle2, Circle, Clock, AlertCircle, Plus, ChevronDown, ChevronRight, Trash2 } from 'lucide-react'
import { formatDate } from '@/lib/utils'
import { updateTaskStatus, createTask, updateMilestoneStatus, deleteTask, deleteMilestone } from '@/lib/actions/projects'
...
function TaskRow({ task, projectId, onStatusChange, onDelete }: {
  task: Task
  projectId: string
  ...
  return (
    ...
    >
      <button onClick={cycleStatus} className="flex-shrink-0 hover:scale-110 transition-transform" title={'Mark as ' + cfg.next}>
        <Icon size={15} style={{ color: cfg.color }} />
      </button>
      <span className={'text-xs flex-1 leading-relaxed ' + (task.status === 'done' || task.status === 'completed' ? 'line-through text-mx-subtle' : 'text-mx-dim')}>
        {task.title}
      </span>
      <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
        {task.estimatedHours && <span className="text-2xs font-mono text-mx-subtle">{task.estimatedHours}h</span>}
        {task.dueDate && <span className="text-2xs font-mono text-mx-subtle">{formatDate(task.dueDate, 'MMM d')}</span>}
        <button onClick={handleDelete} className="text-mx-subtle hover:text-red-400 transition-colors ml-1" title="Delete task">
          <Trash2 size={11} />
        </button>
      </div>
...
function MilestoneSection({ milestone, projectId, users = [], onMilestoneDelete }: {
  milestone: Milestone
  projectId: string
  users?: User[]
  ...
  function cycleMilestone(e: React.MouseEvent) {
    ...
    setMStatus(next)
    startTransition(async () => { await updateMilestoneStatus(milestone.id, projectId, next) })
  }

  function handleMilestoneDelete(e: React.MouseEvent) {
    e.stopPropagation()
  ...
  async function handleAddTask() {
    ...
    startTransition(async () => {
      const result = await createTask({ projectId, milestoneId: milestone.id, title: newTitle.trim(), assignedTo: newAssignee || undefined })
      if (result.success && result.id) {
        const assignedUser = users.find(u => u.id === newAssignee) ?? null
        setTasks(prev => [...prev, {
          id: result.id!, title: newTitle.trim(), status: 'todo',
          estimatedHours: null, dueDate: null,
  ...
  return (
    <div className="card overflow-hidden">
      <button
        ...
        className="w-full px-4 py-3 flex items-center gap-3 hover:bg-mx-muted/30 transition-colors group/header"
        style={{ borderBottom: expanded ? '1px solid #1E1E24' : 'none' }}
      >
        <button onClick={cycleMilestone}
        ...
        />
        {expanded ? <ChevronDown size={13} className="text-mx-mid flex-shrink-0" /> : <ChevronRight size={13} className="text-mx-mid flex-shrink-0" />}
        ...
        <div className="flex items-center gap-3 text-2xs text-mx-mid">
          ...
          {milestone.dueDate && <span className="font-mono">{formatDate(milestone.dueDate, 'MMM d')}</span>}
          {mStatus === 'completed' && <span className="badge" style={{ background: '#22C55E15', color: '#22C55E' }}>Done</span>}
          <button
            onClick={handleMilestoneDelete}
            className="opacity-0 group-hover/header:opacity-100 transition-opacity text-mx-subtle hover:text-red-400 ml-1"
      ...
      {expanded && (
        <div className="px-2 py-1">
          {tasks.length === 0 && !addingTask && <p className="text-2xs text-mx-subtle px-3 py-2">No tasks yet.</p>}
          {tasks.map(task => (
            <TaskRow key={task.id} task={task} projectId={projectId} onStatusChange={handleStatusChange} onDelete={handleTaskDelete} />
          ))}
          {addingTask ? (
            <div className="flex items-center gap-2 px-3 py-2">
              ...
              {users.length > 0 && (
                <select value={newAssignee} onChange={e => setNewAssignee(e.target.value)}
                  className="mx-input text-xs w-28 flex-shrink-0" disabled={isPending}>
              ...
              <input autoFocus value={newTitle} onChange={e => setNewTitle(e.target.value)}
                ...
                className="mx-input flex-1 py-1 text-xs" disabled={isPending} />
            </div>
          ) : (
...
export default function ProjectTaskManager({ projectId, milestones: initialMilestones, users = [] }: Props) {
  ...
  return (
    <div className="space-y-3">
      ...
      {milestones.length === 0 ? (
        ...
        milestones.map(m => (
          <MilestoneSection key={m.id} milestone={m} projectId={projectId} users={users} onMilestoneDelete={handleMilestoneDelete} />
        ))
      )}
    </div>
  )
